 ...
 * We can use less than, greater than or also compare two values like they are
 * equal or not.
 * In Java, equal to (=) is an assignment operator that is used to assign
 * values.
 * x=5 ( here, we are assigning the value 5 to the variable x).
 * Double equal to (==) is used to compare two values.
 * If we have to check the values are not equal, then we use not equal to an
 * operator (!=).
 * 
